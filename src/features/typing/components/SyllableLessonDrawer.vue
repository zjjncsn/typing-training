...
const emit = defineEmits<{
  ...
  select: [lesson: SyllableLessonSummary]
}>()

const expandedCategories = ref<SyllableCategory[]>([])
...
function selectLesson(lesson: SyllableLessonSummary) {
  emit('select', lesson)
  close()
}

function toggleCategory(category: SyllableCategory) {
...
function isExpanded(category: SyllableCategory): boolean {
  ...
  <Teleport to="body">
    <Transition name="drawer">
      <div v-if="open" class="drawer-layer" role="presentation" @click.self="close">
        <aside class="drawer" role="dialog" aria-modal="true" aria-labelledby="syllable-title">
          <header>
            <div>
              <p>拼音打字 · 音节练习</p>
              <h2 id="syllable-title">选择课程</h2>
            </div>
          ...
          <section v-for="group in groups" :key="group.id" class="lesson-group">
            ...
            <ul v-if="isExpanded(group.id)">
              <li v-for="lesson in group.lessons" :key="lesson.id">
                ...
                >
                  <span class="lesson-icon"><span class="pi pi-language" /></span>
                  <span>
                    <strong>{{ lesson.title }}</strong>
                    <small>{{ lesson.entryCount }} 组 · {{ lesson.characterCount }} 字母</small>
                  </span>
